...
vtkStandardNewMacro(CinemaAlgorithm);
CinemaAlgorithm::CinemaAlgorithm() = default;
CinemaAlgorithm::~CinemaAlgorithm() = default;
...
vtkDataSet *CinemaAlgorithm::GetOutput() {
  return this->GetOutput(0);
}

vtkDataSet *CinemaAlgorithm::GetOutput(int port) {
  return vtkDataSet::SafeDownCast(this->GetOutputDataObject(port));
}

void CinemaAlgorithm::SetInputData(vtkDataSet *input) {
  this->SetInputData(0, input);
}

void CinemaAlgorithm::SetInputData(int index, vtkDataSet *input) {
  this->SetInputDataInternal(index, input);
}

void CinemaAlgorithm::AddInputData(vtkDataSet *input) {
  this->AddInputData(0, input);
}

void CinemaAlgorithm::AddInputData(int index, vtkDataSet *input) {
  this->AddInputDataInternal(index, input);
}

int CinemaAlgorithm::RequestDataObject(vtkInformation *request,
  ...
  for(int i = 0; i < this->GetNumberOfOutputPorts(); ++i) {
    ...
    if(outputPortInfo->Has(CinemaAlgorithm::SAME_DATA_TYPE_AS_INPUT_PORT())) {
      // Set output data type to input data type at specified port
      auto inPortIndex
        = outputPortInfo->Get(CinemaAlgorithm::SAME_DATA_TYPE_AS_INPUT_PORT());
...
//==============================================================================
int CinemaAlgorithm::ProcessRequest(vtkInformation *request,
                                 vtkInformationVector **inputVector,
                                 vtkInformationVector *outputVector) {
  // 1. Pass
